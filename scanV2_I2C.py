...
# Constants
ANGLE_RANGE = (0, 100)  # Adjustable up to 180°
ANGLE_STEP = 1
NUM_READINGS = 20
SMOOTH_STEP = 1
SMOOTH_DELAY = 0.02
...
def initialize_hardware(interrupt_pin=board.GP18):
    ...
    return vl53, servo, 0, interrupt, i2c
...
def wait_for_go_signal(i2c=None):
    if i2c is None:
        raise ValueError("I2C object required for I2C go signal")
    ...
    try:
        ...
        while True:
            try:
                ...
                if buffer[0] == 0x01:  # "Go" signal
                    break
            except OSError:  # Slave not ready
                time.sleep(0.1)
    finally:
        i2c.unlock()
...
def send_i2c_data(i2c, center, orientation):
    while not i2c.try_lock():
        pass
    try:
        i2c.writeto(SLAVE_ADDRESS, struct.pack('ffffff', *center, *orientation))
    finally:
        i2c.unlock()

def main():
    vl53, servo, interrupt, i2c = None, None, None, None
    current_angle = None
